...
def features_create(data):
    ...
    # 'CompetitionOpen'：rival open
    # 'PromoOpen' rival promotion open
    # features are in per month
    ...
    month_to_str = {1: 'Jan', 2: 'Feb', 3: 'Mar', 4: 'Apr', 5: 'May', 6: 'Jun', 7: 'Jul', 8: 'Aug', 9: 'Sept', 10: 'Oct',
                 11: 'Nov', 12: 'Dec'}
...
features = ['Store', 'CompetitionDistance', 'CompetitionOpenSinceMonth', 'StateHoliday', 'StoreType', 'Assortment',
...
def objective(trial, train=train):
    params = {
        "lambda": trial.suggest_loguniform('lambda', 0.01, 10.0),  # 0.01,0.1,1,10
...
start=time()
gbm = xgb.train(params, dtrain, num_trees, evals=watchlist, early_stopping_rounds=50, feval=rmspe_xg, verbose_eval=50)
end=time()
...
# submit file
test_probs = gbm.predict(xgb.DMatrix(xtest[features]), ntree_limit=gbm.best_ntree_limit)
