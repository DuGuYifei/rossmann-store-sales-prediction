...
def calculate_outlier(df,column): ## function obtained from course material, added percent_outliers
...
print(str(lower_sales) + ", " + str(upper_sales) +", " + str(percent_outliers_sales) + "%")
...
print(sales[sales[col] > upper_sales])
...
sales_outliers_by_stype.plot(y='Sales', kind='pie', figsize=(6,6), 
...
print(sales_treated[sales_treated['Sales'] > 13612]) ## double-checking our imputation worked, as we can see records of this command are empty
...
print(sales_treated[sales_treated['Customers'] > upper_cust])
...
print(sales_treated[(sales_treated['Customers'] > upper_cust) & (sales_treated['Sales'] == 13612)])
...
cust_outliers_by_stype.plot(y='Customers', kind='pie', figsize=(6,6), 
...
cust_outliers_by_month = pd.pivot_table((sales_treated.loc[sales_treated[col] > upper_cust]), index='Month', values='CompetitionDistance', aggfunc='count')
...
cust_outliers_by_stype = pd.pivot_table((sales_treated.loc[sales_treated[col] > upper_cust]), index='StoreType', values='CompetitionDistance', aggfunc='count')

cust_outliers_by_stype.plot(y='CompetitionDistance', kind='pie', figsize=(6,6), 
...
sales_treated.loc[sales_treated['CompetitionDistance'] > upper_cust, 'CompetitionDistance'] = 16160
