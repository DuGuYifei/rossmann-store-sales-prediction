...
def predict(dataset, storetype, assortment, save_file=False):
	features_create(dataset)
	model_xgb = xgb.Booster()
	model_xgb.load_model("./models/xgboost_modeldata_" + storetype + "_" + assortment + ".json")
	...
	submission = pd.DataFrame(
		#{"Id": dataset["Id"],
		{"Date": dataset["Date"], "Sales": np.expm1(test_probs) * 0.95}
	)
	if save_file:
